              ...
              <div class="p-6 text-gray-900">
                ...
                <div class="text-center flex justify-center">
                  <form id="delete_{{$period->id}}" action="{{route('admin.periods.destroy' , ['period'=>$period->id])}}" method="post">
                    @csrf
                    @method('delete')
                      <button type='submit' data-id="{{$period->id}}}" onclick="deletePost(this)" class="text-black bg-white border-4 transition duration-300 ease-in-out border-red-300 hover:bg-red-200/80 py-2 px-6 focus:outline-none  rounded text-lg mb-2 mx-2 ">削除</button>
                  </form>
                </div>
